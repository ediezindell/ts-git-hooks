...
-   **Configuration Loading**: Finding, reading, and parsing the `ts-git-hooks.config.ts` file. This may involve a `config-loader.ts` that safely `require()`s the config file.
...
-   `config-loader.ts`: Exports a function, `loadConfig()`, that finds `ts-git-hooks.config.ts` in the project root, loads it, and returns the configuration object. It should handle cases where the file doesn't exist.
