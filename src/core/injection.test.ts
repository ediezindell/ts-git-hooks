...
vi.mock('./config', () => ({
    isGlobHookConfig: (config) => true,
    isHookConfigWithOpts: (config) => false,
    loadConfig: vi.fn(),
}));
...
describe('runner injection test with shell-quote', () => {
    it('should be safe from injection when quotes are present', async () => {
        ...
        expect(executable.args).toEqual(['--config', 'conf.json', '$(touch EXPLOITED).ts']);
    });
    ...
    it('should correctly include script name in ArgsFn and be safe', async () => {
        const hookConfig = {
            '*.ts': ['lint', (files, script) => `--files="${files.join(',')}"`]
        };
        const stagedFiles = ['$(touch EXPLOITED).ts'];
    ...
    it('should handle ArgsFn returning full command', async () => {
        const hookConfig = {
            '*.ts': ['lint', (files, script) => `${script} --files="${files.join(',')}"`]
        };
