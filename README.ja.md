...
-   **最小限の依存関係**: `ts-git-hooks` は重要な機能のために、軽量でメンテナンスされた3つの依存関係のみを使用します。

## 要件

-   **Node.js**: v22.6.0 以上 (ネイティブ TypeScript サポートのため)
-   **Git**: v2.25 以上 (`git add` にファイルリストを `--pathspec-from-file=-` 経由で渡し、OS の argv 長制限を回避するため)
